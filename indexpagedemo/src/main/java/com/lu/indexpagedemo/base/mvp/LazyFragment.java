...
public abstract class LazyFragment extends SupportFragment {
    ...
    public void onDetach() {
        super.onDetach();
        //防止ViewPager的直接调用,对于ViewPage在MVP架构时，Presenter会获得View的引用（非强引用）当View
        //被销毁后，该引用会为null.而使用ViewPager在管理fragment时，会直接通过fragmentManger间接调用fragment的
        //onDetach和onAttach，setUserVisible函数,当使用EventBus时，传递异步消息到多个View（View中会接收和视图相关的信息）时，
        // 中间存在延迟,如果快速切换ViewPager,相邻的fragment（他们被预加载过但任然是不可见状态）,会直接调用onDetach
        // 函数
